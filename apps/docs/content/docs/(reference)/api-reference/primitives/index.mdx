---
title: 原语 API 参考
description: "用于 assistant-ui 聊天 UI 的可组合 React 原语：Thread、Composer、Message、BranchPicker、ActionBar，以及构建线程、消息列表、附件和编辑流程的各个部分。"
---
...
{/* api-manual:start */}
原语是 assistant-ui 界面背后的底层 React 组件。它们提供无障碍行为、状态连接和组合扩展点，同时将布局与样式的控制权交给你。
{/* api-manual:end */}
